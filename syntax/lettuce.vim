...
" Filenames:    *.feature
" Last Change:	2010 Sep 12
" This syntax file is based on Cucumber syntax file by Tim Pope
...
function! s:pattern(key)
  ...
  if has_key(g:lettuce_languages, language)
    let languages = [g:lettuce_languages[language]]
  else
    let languages = values(g:lettuce_languages)
  end
...
function! s:Add(name)
  ...
  exe "syn region lettuce".a:name.'Region matchgroup=lettuce'.a:name.' start="\%(^\s*\)\@<=\%('.s:pattern(tolower(a:name)).'\)" end="$"'.next
  exe 'syn region lettuce'.a:name.'AndRegion matchgroup=lettuce'.a:name.'And start="\%(^\s*\)\@<='.s:pattern('and').'" end="$" contained'.next
  exe 'syn region lettuce'.a:name.'ButRegion matchgroup=lettuce'.a:name.'But start="\%(^\s*\)\@<='.s:pattern('but').'" end="$" contained'.next
  exe 'syn match lettuce'.a:name.'Comment "\%(^\s*\)\@<=#.*" contained'.next
  exe 'syn match lettuce'.a:name.'Table "\%(^\s*\)\@<=|.*" contained contains=lettuceDelimiter'.next
  exe 'hi def link lettuce'.a:name.'Comment lettuceComment'
  exe 'hi def link lettuce'.a:name.'But lettuce'.a:name.'And'
  exe 'hi def link lettuce'.a:name.'And lettuce'.a:name
  exe 'syn cluster lettuceStepRegions add=lettuce'.a:name.'Region,lettuce'.a:name.'AndRegion,lettuce'.a:name.'ButRegion'
endfunction

syn match   lettuceComment  "\%(^\s*\)\@<=#.*"
syn match   lettuceComment  "\%(\%^\s*\)\@<=#.*" contains=lettuceLanguage
syn match   lettuceLanguage "\%(#\s*\)\@<=language:" contained
syn match   lettuceUnparsed "\S.*" nextgroup=lettuceUnparsedComment,lettuceUnparsed,lettuceTags,lettuceBackground,lettuceScenario,lettuceScenarioOutline,lettuceExamples skipwhite skipempty contained
syn match   lettuceUnparsedComment "#.*" nextgroup=lettuceUnparsedComment,lettuceUnparsed,lettuceTags,lettuceBackground,lettuceScenario,lettuceScenarioOutline,lettuceExamples skipwhite skipempty contained

exe 'syn match lettuceFeature "\%(^\s*\)\@<='.s:pattern('feature').':" nextgroup=lettuceUnparsedComment,lettuceUnparsed,lettuceBackground,lettuceScenario,lettuceScenarioOutline,lettuceExamples skipwhite skipempty'
exe 'syn match lettuceBackground "\%(^\s*\)\@<='.s:pattern('background').':"'
exe 'syn match lettuceScenario "\%(^\s*\)\@<='.s:pattern('scenario').':"'
exe 'syn match lettuceScenarioOutline "\%(^\s*\)\@<='.s:pattern('scenario_outline').':"'
exe 'syn match lettuceExamples "\%(^\s*\)\@<='.s:pattern('examples').':" nextgroup=lettuceExampleTable skipempty skipwhite'
...
syn match   lettucePlaceholder   "<[^<>]*>" contained containedin=@lettuceStepRegions
syn match   lettuceExampleTable  "\%(^\s*\)\@<=|.*" contains=lettuceDelimiter,lettuceNumber,lettuceString,lettuceFloat
syn match   lettuceDelimiter     "|" contained
syn match   lettuceTags          "\%(^\s*\)\@<=\%(@[^@[:space:]]\+\s\+\)*@[^@[:space:]]\+\s*$"

syn region  lettuceString   start='"' end='"' containedin=@lettuceStepRegions
syn match   lettuceNumber "\<\d\+[lLjJ]\=\>" containedin=@lettuceStepRegions,
syn match   lettuceFloat "\.\d\+\([eE][+-]\=\d\+\)\=[jJ]\=\>" containedin=@lettuceStepRegions
syn match   lettuceFloat "\<\d\+[eE][+-]\=\d\+[jJ]\=\>" containedin=@lettuceStepRegions
syn match   lettuceFloat "\<\d\+\.\d*\([eE][+-]\=\d\+\)\=[jJ]\=" containedin=@lettuceStepRegions

hi def link lettuceUnparsedComment   lettuceComment
hi def link lettuceComment           Comment
hi def link lettuceLanguage          SpecialComment
hi def link lettuceFeature           Statement
hi def link lettuceBackground        Define
hi def link lettuceScenario          Define
hi def link lettuceScenarioOutline   Define
hi def link lettuceExamples          Define
hi def link lettucePlaceholder       Constant
hi def link lettuceDelimiter         Delimiter
hi def link lettuceTags              Tag
hi def link lettuceString            String
hi def link lettuceGiven             Function
hi def link lettuceWhen              Function
hi def link lettuceThen              Function
hi def link lettuceNumber            Number
hi def link lettuceFloat             Float

let b:current_syntax = "lettuce"
